-- Camlock.lua
local Config = shared.Ecco['Camlock'] -- read all settings from the config
...
-- Finds nearest target based on config
local function GetNearestTarget()
    ...
    for i, v in pairs(players) do
        ...
        if AIM then
            if not Config.TeamCheck or v.Team ~= Client.Team then
...
RunService.RenderStepped:Connect(function()
    if Config.ThirdPerson or Config.FirstPerson then CanNotify = true end
    ...
    if Aimlock and MousePressed and AimlockTarget and AimlockTarget.Character and AimlockTarget.Character:FindFirstChild(Config.AimPart) then
        ...
        if Config.Smoothness then
            Camera.CFrame = Camera.CFrame:Lerp(CF(Camera.CFrame.Position, TargetPos), Config.SmoothnessAmount, Enum.EasingStyle.Elastic, Enum.EasingDirection.InOut)
        else
            Camera.CFrame = CF(Camera.CFrame.Position, TargetPos)
        end
    end

    -- Check if target is airborne and swap aim part
